...
res_data <- matrix(scan(opt$file), byrow=TRUE, ncol=25)

pdf(paste("human_data.pdf"))
human_data <- sqrt(res_data[,14]*res_data[,14]*theta[1]*biting_rate*area_convert*vdensity/rabundance[1]/vmu*theta[1]*biting_rate/(rgamma[1]+rmu[1]))
...
for (i in 1:12)  {
  species_data<- sqrt(res_data[,13+i]*res_data[,13+i]*theta[i]*biting_rate*area_convert*vdensity/rabundance[i]/vmu*theta[i]*biting_rate/(rgamma[i]+rmu[i]))
...
for (i in 2:4) {
  species_data <- species_data +
    res_data[,i+13]*res_data[,i+13]*theta[i]*biting_rate*area_convert*vdensity/rabundance[i]/vmu*theta[i]*biting_rate/(rgamma[i]+rmu[i])
}
